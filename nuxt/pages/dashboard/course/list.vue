<template>
  <div class="">
    <div class="container">
      <div class="mt-12">
        <h1 class="text-xl">دوره های آموزشی من</h1>

        <hr class="mx-28 mt-6 mb-6" />

        <section
          v-if="myCoursesQuery.result.value && myCoursesQuery.result.value.user"
          class="body-font overflow-hidden text-gray-600"
        >
          <div
            v-for="course in myCoursesQuery.result.value.user.courses"
            :key="course.id"
            class="mx-auto py-10"
          >
                ...
                <div class="flex h-full w-4/5 flex-col justify-evenly">
                  ...
                  <div>
                    <nuxt-link
                      ...
                      class="flex items-center justify-between rounded bg-indigo-50 px-4 py-2 text-indigo-500"
                    >
          ...
          <div
            v-if="myCoursesQuery.result.value.user.courses?.length === 0"
            class="text-black"
          >
            fa:: you have none
          </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const myCoursesQuery = useLazyQuery<
      MyCoursesQuery,
      MyCoursesQueryVariables
    >(MYCOURSESQUery, { id: userID })

    const tab = computed(() => (ctx.route.value.query.tab as string) || '')
